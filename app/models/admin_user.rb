class AdminUser < ApplicationRecord
  ...
  def self.send_otp options
    ...
    options.merge!({ code: AdminUser.otp, token: token })
  ...
  def create_validations
    if self.email.blank?
      ...
      errors.add(:email, I18n.t('auth.already_exist', key: :email, value: self.email)) if AdminUser.exists?(email: self.email)
    end
  ...
  def clear_cache
    Core::Redis.delete(Core::Redis::ADMIN_USER_BY_ID % { id: self.id })
  end

  def clear_sessions
    if self.saved_change_to_password_digest? || (self.saved_change_to_two_fa_enabled? && self.two_fa_enabled)
      AdminUserSession.where(admin_user_id: self.id).delete_all
      Core::Redis.delete(AdminUserSession.cache_key(self.id))
    end
  end
end
