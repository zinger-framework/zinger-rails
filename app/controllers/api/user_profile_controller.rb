class Api::UserProfileController < ApiController
  ...
  def reset_password
    if params['current_password'].blank? || Customer.current.auth_mode != Customer::AUTH_MODE['PASSWORD_AUTH'] || 
      ...
      render status: 401, json: { success: false, message: I18n.t('customer.password.update_failed'), reason: { 
        current_password: [I18n.t('validation.invalid', param: 'password')] } }
      return
    end

    if params['new_password'].blank?
      render status: 400, json: { success: false, message: I18n.t('customer.password.update_failed'), reason: { 
    ...
    if Customer.current.errors.any?
      render status: 401, json: { success: false, message: I18n.t('customer.password.update_failed'), reason: Customer.current.errors.messages }
      return
    end
