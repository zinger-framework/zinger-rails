class Admin::AuthController < AdminController
  ...
  before_action :go_to_dashboard, only: [:index, :login, :register, :signup, :forgot_password, :reset_password]
  before_action :verify_jwt_token, only: [:otp, :otp_login, :resend_otp]
